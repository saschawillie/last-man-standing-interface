...
export default function WrapButton({ setAccount, onConnect, width }) {
  ...
  const query = useQuery(
    ...
    },
    {
      ...
      enabled: !!connectedAccount && !!tokenContract && !!underlyingContract,
    }
  );

  const wrap = useMutation(
    async () => {
      if (wrapValue <= 0) return;
      const approval = await underlyingContract.approve(
        WRAPPING_TOKEN_CONTRACT_ADDRESS,
        BigInt(wrapValue * Math.pow(10, decimals)),
        {
      ...
      const txn = await tokenContract.wrap(BigInt(wrapValue * Math.pow(10, decimals)), {
        gasLimit: BigInt(1000000),
      });
      await txn.wait();
    },
